...
CREATE OR ALTER VIEW [Undelivered Pizzas] AS
	SELECT p.Name, p.SizeId, p.OrderId, dbo.GetFullName(u.FirstName, u.LastName) AS [User] 
	FROM Pizzas AS p
	JOIN Orders AS o
	ON o.Id = p.OrderId
	JOIN Users AS u
...
BEGIN
	...
	SELECT @pizzasPrice = SUM(p.Price), @toppingsPrice = SUM(t.Price) 
	FROM Orders AS o
	JOIN Pizzas AS p
	ON o.Id = p.OrderId
...
EXEC dbo.GetFullPriceOfOrder @OrderId = 5
GO
...
CREATE OR ALTER VIEW [Pizza Popularity] 
AS
	SELECT 
		p.Name, 
...
SELECT 
	...
	COUNT(*) AS [Number of times ordered]
FROM Toppings AS t
JOIN PizzaToppings AS pt ON t.Id = pt.ToppingId
GROUP BY t.Name
GO
